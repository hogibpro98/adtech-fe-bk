...
import BannerTeloChart from "~/components/page/manage-ads/vendor/index/BannerTeloChart.vue";
...
>).extend({
  ...
  methods: {
    ...
    async fetchDataBanner(bannerId) {
      try {
        this.loading = true;
        const res = await this.$adsSmsVendorRepository.getDetail(bannerId);
        this.loading = false;
        if (res.code === CODE_SUCCESS) {

<template>
  <v-container grid-list-xl fluid class="no-padding no-margin">
      ...
      <v-flex lg12>
        <v-card>
          <v-card-text style="color: #333333">
            <em>Lĩnh vực hoạt động được phép đồng bộ</em>
            <v-divider></v-divider>
          </v-card-text>
          <api-data-table
            ref="table"
    ...
    ></catid-config-dialog>
    <catid-config-delete-dia-log
      ref="deleteDialog"
      @action-success="refreshTable"
    ></catid-config-delete-dia-log>
    <base-mass-delete-dialog
      ref="massDeleteDialog"
      resource="/sync-manual-cat-config"
      object-name="cấu hình lĩnh vực hoạt động"
...
import { SYNC_MANUAL_CAT_CONFIG } from "@/constants/resource";
...
import CatidConfigDialog, {
  CatidConfigDialogRef
} from "~/components/page/manage-sync-ads/CatidConfig/CatidConfigDialog.vue";
import CatidConfigDeleteDiaLog, {
  CatidConfigDeleteDiaLogRef
} from "~/components/page/manage-sync-ads/CatidConfig/CatidConfigDeleteDiaLog.vue";
...
interface ComponentData {
  ...
  $refs: {
    ...
    dialog: CatidConfigDialogRef;
    deleteDialog: CatidConfigDeleteDiaLogRef;
  };
...
>).extend({
  ...
  data() {
    return {
      ...
      configs: {},
      loading: false,
      resource: SYNC_MANUAL_CAT_CONFIG,
  ...
  methods: {
    async fetchConfig() {
      this.loading = true;
      const { data } = await this.$axios.$post("sync-manual-cat-config/get-apply");
      if (data.length > 0) {
        this.first = false;
        this.configs = data;
      }

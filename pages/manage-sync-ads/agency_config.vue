<template>
  <v-container grid-list-xl fluid class="no-padding no-margin">
    ...
    <base-mass-delete-dialog
      ...
      resource="/sync-manual-agency-config"
      object-name="cấu hình Agency"
...
import { SYNC_MANUAL_AGENCY_CONFIG } from "@/constants/resource";
...
import FormDiaLog, {
  FormDiaLogRef
} from "~/components/page/manage-sync-ads/AgencyConfig/FormDiaLog.vue";
import DeleteDiaLog, {
  DeleteDiaLogRef
} from "~/components/page/manage-sync-ads/AgencyConfig/DeleteDiaLog.vue";
...
>).extend({
  ...
  data() {
    return {
      ...
      configs: {},
      loading: false,
      resource: SYNC_MANUAL_AGENCY_CONFIG,
  ...
  methods: {
    async fetchConfig() {
      ...
      const { data } = await this.$axios.$post(
        "sync-manual-agency-config/get-apply"
      );
      if (data.length > 0) {
        this.first = false;
        this.configs = data;

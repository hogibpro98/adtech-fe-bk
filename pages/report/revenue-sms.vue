      ...
      <v-flex lg12>
        ...
        >
          ...
          <template slot="item.created_at" slot-scope="props">
            <div class="text-center">{{ props.item.created_at | date }}</div>
          </template>

          <template slot="item.price" slot-scope="props">
            <div class="text-right">{{ props.item.price | currency }}</div>
          </template>
          ...
          <template v-if="summary" #body.prepend="{ headers, isMobile }">
            <tr v-if="!isMobile">
              ...
              <template v-for="(header, index) in columnShows">
                <td
                  v-if="index !== 0"
                  :key="`revenue-sms-column-prepend-${index}`"
                  style="border-right: none!important;"
...
import { SMS_DAY } from "~/constants/resource";
...
export default {
  ...
  methods: {
    ...
    async getSummary(params) {
      try {
        const { data } = await this.$SMSDayRepository.post("summary", params);
        this.summary = data;
      } catch (e) {
        console.log(e);
      }

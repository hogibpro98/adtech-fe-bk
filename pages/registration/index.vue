          ...
          <v-flex xs12 sm10 md8 lg5 xl4>
            <v-card class="elevation-1 pa-3" light>
              <v-card-text>
                ...
                <h2 class="flex my-6 blue--text text-center">Đăng ký tài khoản</h2>
              </v-card-text>
              <v-layout align-center justify-space-between wrap>
                <v-flex
                  :class="{
                    'darken-1': hoverItem === 'agency'
                  }"
                  xs6
                  class="purple white--text d-flex flex-column align-center justify-content-center pointer pa-4"
                  @mouseover="hoverItem = 'agency'"
                  @mouseleave="hoverItem = null"
                  @click="goToAgency"
                >
                  <v-icon size="48" color="white">support_agent</v-icon>
                  <h2>Agency</h2>
                </v-flex>
                <v-flex
                  :class="{
                    'darken-1': hoverItem === 'publisher'
                  }"
                  xs6
                  class="green white--text d-flex flex-column align-center justify-content-center pointer pa-4"
                  @mouseover="hoverItem = 'publisher'"
                  @mouseleave="hoverItem = null"
                  @click="goToPublisher"
                >
                  <v-icon size="48" color="white">mdi-newspaper-variant-outline</v-icon>
                  <h2>Publisher</h2>
                </v-flex>
              </v-layout>
...
>).extend({
  ...
  methods: {
    ...
    goToAgency() {
      this.$router.push("/registration/agency");
    },
    goToPublisher() {
      this.$router.push("/registration/publisher");
    },

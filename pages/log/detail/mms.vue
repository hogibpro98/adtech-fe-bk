<template>
  <v-container grid-list-xl fluid class="no-padding no-margin">
    <header-select-time
      v-model="date"
      title="Chi tiết log tin nhắn MMS"
      class="sticky-header"
    />
    <v-layout row wrap>
      <v-flex lg12>
          ...
          >
            <template slot="item.ads" slot-scope="{ item }">
              <a
                ...
                >{{ item.ads.name }}</a
              >
              <span v-else>Tin nhắn MMS đã bị xóa</span>
            </template>
    ...
    <mms-log-dialog ref="dialog" title="Chi tiết thay đổi"></mms-log-dialog>
  </v-container>
</template>
<script lang="ts">
  ...
  props: {
    ...
    toolbar: {
      ...
      default: () => ({
        ...
        isFullScreen: false,
        isDownload: true,
        exportResource: "/log/mms-detail-log"
      })
    },
    headerExcludes: {
      type: Array,

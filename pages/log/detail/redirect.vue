<template>
  <v-container grid-list-xl fluid class="no-padding no-margin">
    <header-select-time
      v-model="date"
      title="Chi tiết log quảng cáo redirect"
      class="sticky-header"
    />
    <v-layout row wrap>
      <v-flex lg12>
          ...
          >
            <template slot="item.ads" slot-scope="{ item }">
              <a
                ...
                >{{ item.ads.name }}</a
              >
              <span v-else>Quảng cáo redirect đã bị xóa</span>
            </template>
            <template slot="item.date" slot-scope="{ item }">{{ item.date | date }}</template>
          </api-data-table>
        </v-card>
      </v-flex>
    </v-layout>
...
import MmsLogDialog from "~/components/page/log/detail/mms/MmsLogDialog.vue";
import RedirectLogDialog from "~/components/page/log/detail/redirect/RedirectLogDialog.vue";
...
>).extend({
  ...
  props: {
    resource: {
      type: String,
      default: "/log/redirect-detail-log"
    },
    toolbar: {
      type: Object,
      default: () => ({
        ...
        isFullScreen: false,
        isDownload: true,
        exportResource: "/log/redirect-detail-log"
      })
    },
    headerExcludes: {
      type: Array,
  ...
  methods: {
    ...
    showAds(item) {
      this.$router.push(`/manage-ads/sms?select_id=${item.ads_id}`);
    }
  }
});
</script>

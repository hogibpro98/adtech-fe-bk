<template>
  <v-container grid-list-xl fluid class="no-padding no-margin">
    <header-select-time
      v-model="date"
      title="Chi tiết log tài trợ data"
      class="sticky-header"
    />
    <v-layout row wrap>
      <v-flex lg12>
          ...
          >
            <template slot="item.ads" slot-scope="{ item }">
              <a
                ...
                >{{ item.ads.name }}</a
              >
              <span v-else>Tài trợ data đã bị xóa</span>
            </template>
    ...
    <data-log-dialog ref="dialog" title="Chi tiết thay đổi"></data-log-dialog>
  </v-container>
</template>
<script lang="ts">
...
>).extend({
  ...
  props: {
    resource: {
      type: String,
      default: "/log/data-sponsor-detail-log"
    },
    toolbar: {
      type: Object,
      default: () => ({
        ...
        isFullScreen: false,
        isDownload: true,
        exportResource: "/log/data-sponsor-detail-log"
      })
    },
    headerExcludes: {
      type: Array,
  ...
  methods: {
    ...
    showAds(item) {
      this.$router.push(`/manage-ads/data?select_id=${item.ads_id}`);
    }
  }
});
</script>

      ...
      <v-flex lg12>
        <v-card>
          ...
          <api-data-table
            ...
            :toolbar="toolbar"
            :fixed-left-count="2"
            page-name="log-mms-distribution"
            calculate-widths
          ...
          >
            ...
            <template slot="item.content" slot-scope="{ item }">
              <a
                ...
                >{{ textContent(item) || "Xem chi tiết" }}</a
              >
            ...
            <template slot="item.num_retry" slot-scope="{ item }">{{
              item.num_retry | numberSpace
            }}</template>
  ...
  props: {
    ...
    toolbar: {
      ...
      default: () => ({
        ...
        isFullScreen: false,
        isDownload: true,
        exportResource: "/log/mms-distribution"
      })
    },
    filterCustom: {
      type: Object,

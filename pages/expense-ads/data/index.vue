  ...
  <v-container grid-list-xl fluid class="no-padding no-margin">
    <v-layout row wrap>
      <v-flex lg12>
        ...
        <expense-data-detail-dialog ref="expenseSMSDetailDialog" />
      </v-flex>
    </v-layout>
    <export-dialog
      ref="exportDialog"
...
import { TRANSACTION_DATA_DAILY } from "@/constants/resource";
...
import ExpenseDataDetailDialog from "~/components/page/expense-ads/data/ExpenseDataDetailDialog.vue";
...
>).extend({
  ...
  props: {
    resource: {
      type: String,
      default: TRANSACTION_DATA_DAILY
    },
    toolbar: {
      type: Object,
      default: () => ({
  ...
  watch: {
    filterTable: {
      handler(val) {
        console.log("changed");
        this.debounceSummary();
      },
      deep: true
    }
  },
  created() {
    this.debounceSummary = debounce(() => {
    ...
    async getSummary(params) {
      try {
        const { data } = await this.$axios.post(
          `${TRANSACTION_DATA_DAILY}/summary`,
          params
        );

        this.totalCharge = data?.data?.total_charge;
        this.totalDebt = data?.data?.total_debt;
        this.totalRecharge = data?.data?.total_recharge;
      } catch (e) {}
    },

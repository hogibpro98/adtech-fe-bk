...
const HeaderGroupSelect = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "HeaderGroupSelect",
  props: {
    headers: {
      type: Array,
      default: () => []
  ...
  computed: {
    headerValue() {
      if (this.fixedHeader) {
        return this.headerLabel || this.innerValue?.text;
      } else {
  ...
  methods: {
    ...
    setValue(item) {
      ...
      this.$emit("select-attribute", item);
    }
  }
});

export default HeaderGroupSelect;
export type HeaderGroupSelectRef = InstanceType<typeof HeaderGroupSelect>;
</script>

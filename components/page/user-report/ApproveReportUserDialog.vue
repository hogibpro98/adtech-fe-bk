<template>
  <confirm-dialog
    ...
    :loading="loading"
    title="Bạn muốn phê duyệt phản hồi người dùng?"
    label-button-confirm="Gửi"
    @confirm="confirmItem"
  >
    <template slot="content">
      Bạn hãy mô tả lý do phê duyệt phản hồi của người dùng
...
const ApproveReportUserDialog = (Vue as VueConstructor<
    ...
    async confirmItem() {
      try {
        ...
        if (res.code === CODE_SUCCESS) {
          this.$emit("confirm");
          this.$toast.global.action_success("phê duyệt");
          this.hide();
        } else {
...
export default ApproveReportUserDialog;
export type ApproveReportUserDialogRef = InstanceType<
  typeof ApproveReportUserDialog
>;
</script>

...
const ManageSyncAdsDialog = Vue.extend<
  ComponentData,
  ComponentMethods,
  ComponentComputed,
  ComponentProps
>({
  name: "ManageSyncAdsDialog",
  components: {
  ...
  computed: {
    title() {
      if (this.isEdit) {
        return "Cập nhật dữ liệu đồng bộ";
      } else {
        return "Thêm mới dữ liệu đồng bộ";
      }
    },
    labelButtonConfirm() {
      if (this.isEdit) {
    ...
    async editItem() {
      ...
      try {
        const result = await this.$axios.$post(
          "sync-manual-summary-day/update",
          this.defaultItem
        );
...
export default ManageSyncAdsDialog;
</script>

...
interface ComponentData {
  ...
  $agencyRepository: RepositoryInterface;
...
const FormDiaLog = (Vue as VueConstructor<
  ...
  methods: {
    show(item = null) {
      ...
      this.title = `${this.isEdit ? "Cập nhật" : "Thêm"} cấu hình Agency`;
    ...
    async addItem() {
      ...
      try {
        this.mapData();
        const res = await this.$agencyRepository.create(this.item);
        this.loading = false;
        if (res.code === CODE_SUCCESS) {
          this.hidden();
          this.$emit("create-success");
          this.$toast.global.add_success("cấu hình Agency");
        } else {
    ...
    async editItem() {
      ...
      try {
        this.mapData();
        const res = await this.$agencyRepository.update(this.item);
        this.loading = false;
        if (res.code === CODE_SUCCESS) {
          this.hidden();
          this.$emit("update-success");
          this.$toast.global.edit_success("cấu hình Agency");
        } else {
          this.$toast.global.action_fail();
        }
      } catch (e) {

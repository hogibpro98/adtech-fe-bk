<template>
  <confirm-dialog
    ...
    title="Bạn muốn xóa cấu hình Agency?"
    label-button-confirm="Xác nhận"
    :loading="loading"
    @confirm="deleteItem"
  >
    <template slot="content">
      Bạn có chắc chắn muốn xóa cấu hình Agency
...
/**
 * @property {Object} $agencyRepository
 */
...
>).extend({
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$agencyRepository.delete({
          id: this.item.id
        });
        this.loading = false;
        if (res.code === CODE_SUCCESS) {
          ...
          this.$toast.global.delete_success("cấu hình Agency");
        } else {

<template>
  <confirm-dialog
    ...
    :loading="loading"
    title="Xóa sub-inventory"
    label-button-confirm="Xác nhận"
    @confirm="deleteItem"
  >
    <template slot="content">
      Bạn có chắc chắn muốn xóa
      <span class="red--text">{{ items.length }}</span> sub-inventory không?
      <div>Danh sách sub-inventory sẽ bị xóa:</div>
      <ul>
        <li
...
const ChannelMassDeleteDialog = (Vue as VueConstructor<Vue & ComponentData & ComponentMethod>).extend({
  name: "ChannelMassDeleteDialog",
    ...
    displayWarning() {
      if (this.items.length > 0) {
        ...
        if (this.items.length > 1) {
          return `${firstName} và ${this.items.length - 1} sub-inventory khác`;
        }
        return firstName;
      }
      return "";
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$channelRepository.massDelete({
          ids: this.ids
        });
        this.loading = false;
...
export default ChannelMassDeleteDialog;
export type ChannelMassDeleteDialogRef = InstanceType<
  typeof ChannelMassDeleteDialog
  >;
</script>

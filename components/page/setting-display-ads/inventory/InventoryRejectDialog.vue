<template>
  ...
  >
    <template slot="content">
      Bạn có chắc chắn muốn từ chối phê duyệt inventory
...
const InventoryRejectDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "InventoryRejectDialog",
  ...
  methods: {
    ...
    async confirmItem(reason) {
      this.loading = true;
      try {
        const res = await this.$inventoryDisplayRepository.post("reject", {
          id: this.item.id,
          status: "reject",
          reason
        });
        ...
        if (res.code === CODE_SUCCESS) {
          ...
          this.$toast.global.action_success("từ chối phê duyệt inventory");
        } else {
...
export default InventoryRejectDialog;
export type InventoryRejectDialogRef = InstanceType<
  typeof InventoryRejectDialog
>;
</script>

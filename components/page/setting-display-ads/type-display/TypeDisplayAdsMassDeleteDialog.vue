<template>
  ...
  >
    <template slot="content">
      ...
      <span class="red--text">{{ items.length }}</span> loại hiển thị quảng cáo không?
      <div>Danh sách loại hiển thị quảng cáo sẽ bị xóa:</div>
      <ul>
        <li
...
const TypeDisplayAdsMassDeleteDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
  >).extend({
  name: "TypeDisplayAdsMassDeleteDialog",
    ...
    displayWarning() {
      if (this.items.length > 0) {
        ...
        if (this.items.length > 1) {
          return `${firstName} và ${this.items.length - 1} loại khác`;
        }
        return firstName;
      }
      return "";
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$adsTypeRepository.massDelete({
          ids: this.ids
        });
        this.loading = false;
...
export default TypeDisplayAdsMassDeleteDialog;
export type TypeDisplayAdsMassDeleteDialogRef = InstanceType<
  typeof TypeDisplayAdsMassDeleteDialog
  >;
</script>

<template>
  ...
  >
    <v-card>
      <v-card-title>
        <span class="headline pt-3">Xóa hoá đơn</span>
      </v-card-title>
      <v-card-text>
...
const InvoiceServiceDeleteDialog = (Vue as VueConstructor<
    ...
    async deleteItem() {
      ...
      try {
        ...
        if (res.code === CODE_SUCCESS) {
          ...
          this.$toast.global.delete_success("hóa đơn");
        } else {
...
export type InvoiceServiceDeleteDialogRef = InstanceType<
  typeof InvoiceServiceDeleteDialog
  >;
</script>

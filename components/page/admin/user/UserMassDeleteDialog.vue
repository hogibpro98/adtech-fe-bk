<template>
  ...
  >
    <template slot="content">
      ...
      <div>Danh sách tài khoản sẽ bị xóa:</div>
      <ul>
        <li
...
const UserMassDeleteDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "UserMassDeleteDialog",
    ...
    displayWarning() {
      if (this.items.length > 0) {
        ...
        if (this.items.length > 1) {
          return `${firstName} và ${this.items.length - 1} user khác`;
        }
        return firstName;
      }
      return "";
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$userRepository.massDelete({
          ids: this.ids
        });
        this.loading = false;
        if (res.code === CODE_SUCCESS) {
          ...
          this.$toast.global.delete_success("tài khoản");
        } else {
...
export default UserMassDeleteDialog;
export type UserMassDeleteDialogRef = InstanceType<typeof UserMassDeleteDialog>;
</script>

<template>
  <form-dialog
    ...
    title="Tải lên dữ liệu các thiết bị"
...
const TargetingDeviceImportFormDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "TargetingDeviceImportFormDialog",
  components: {
    FileUpload,
  ...
  props: {
    sampleUrl: {
      type: String,
      default: "/targeting/templates/Template_Import_tên_thiết_bị.xlsx"
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      try {
        ...
        const res = await this.$deviceRepository.post("import", data);
        this.loading = false;
...
export default TargetingDeviceImportFormDialog;
export type TargetingDeviceImportFormDialogRef = InstanceType<
  typeof TargetingDeviceImportFormDialog
>;
</script>

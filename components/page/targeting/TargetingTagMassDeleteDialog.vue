<template>
  ...
  >
    <template slot="content">
      ...
      <span class="red--text">{{ items.length }}</span> tag không?
      <div>Danh sách tag sẽ bị xóa:</div>
      <ul>
        <li
...
const TargetingTagMassDeleteDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "TargetingTagMassDeleteDialog",
    ...
    displayWarning() {
      if (this.items.length > 0) {
        ...
        if (this.items.length > 1) {
          return `${firstName} và ${this.items.length - 1} tags khác`;
        }
        return firstName;
      }
      return "";
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$targetingTagRepository.massDelete({
          ids: this.ids
        });
        this.loading = false;
        if (res.code === CODE_SUCCESS) {
          ...
          this.$toast.global.delete_success("tag");
        } else {
...
export default TargetingTagMassDeleteDialog;
export type TargetingTagMassDeleteDialogRef = InstanceType<
  typeof TargetingTagMassDeleteDialog
>;
</script>

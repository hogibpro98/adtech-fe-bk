<template>
  <form-dialog
    ...
    title="Tải lên dữ liệu các gói dữ liệu"
...
const TargetingNetworkDataImportFormDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "TargetingNetworkDataImportFormDialog",
  components: {
    FileUpload,
  ...
  props: {
    sampleUrl: {
      type: String,
      default: "/targeting/templates/Template_Import_gói_dữ_liệu.xlsx"
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      try {
        ...
        const res = await this.$networkDataRepository.post("import", data);
        this.loading = false;
...
export default TargetingNetworkDataImportFormDialog;
export type TargetingNetworkDataImportFormDialogRef = InstanceType<
  typeof TargetingNetworkDataImportFormDialog
>;
</script>

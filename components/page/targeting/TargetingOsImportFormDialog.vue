<template>
  <form-dialog
    ...
    title="Tải lên dữ liệu các hệ điều hành"
...
const TargetingOsImportFormDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "TargetingOsImportFormDialog",
  components: {
  ...
  props: {
    sampleUrl: {
      type: String,
      default: "/targeting/templates/Template_Import_hệ_điều_hành.xlsx"
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      try {
        ...
        const res = await this.$osRepository.post("import", data);
        this.loading = false;
...
export default TargetingOsImportFormDialog;
export type TargetingOsImportFormDialogRef = InstanceType<
  typeof TargetingOsImportFormDialog
>;
</script>

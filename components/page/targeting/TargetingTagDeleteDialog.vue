...
const TargetingTagDeleteDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "TargetingTagDeleteDialog",
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$targetingTagRepository.delete({
          id: this.item.id
        });
        this.loading = false;
...
export default TargetingTagDeleteDialog;
export type TargetingTagDeleteDialogRef = InstanceType<
  typeof TargetingTagDeleteDialog
>;
</script>

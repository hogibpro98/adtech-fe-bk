<template>
  ...
  >
    <div class="text-h6">
      <v-tabs v-model="tab">
        <v-tab v-for="item in visibleTabItems" :key="item.value">
          {{ item.text }}
        </v-tab>
      </v-tabs>
    </div>
...
import { ListItemInterface } from "~/interface/data/common";

interface ComponentData {
  id: string;
  innerValue: Array<string>;
  tabItems: Array<ListItemInterface>;
  visibleTabItems: Array<ListItemInterface>;
  tab: string;
}
...
const DashboardHeader = (Vue as VueConstructor<
...
>).extend({
  ...
  computed: {
    ...
    visibleTabItems() {
      return this.tabItems.filter(e => {
        return this.availableTypes.includes(e.value);
      });
    }
  },
  watch: {
    ...
    type(newVal) {
      this.tab = this.visibleTabItems.findIndex(e => e.value === newVal);
    },

    tab(newVal) {
      this.$emit(
        "update:type",
        this.visibleTabItems[newVal] ? this.visibleTabItems[newVal].value : 0
      );
    }
    // minDate(val) {
  ...
  created() {
    this.tab = this.visibleTabItems.indexOf(e => e.value == this.type);
    this.innerValue = this.$filters.sqlDate([
      Date.now() - 6 * 24 * 3600 * 1000,
      Date.now()
    ]);
...
export type DashboardHeaderRef = InstanceType<typeof DashboardHeader>;
</script>

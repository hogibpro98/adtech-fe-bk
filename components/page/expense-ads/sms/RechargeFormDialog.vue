...
const RechargeFormDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "RechargeFormDialog",
  components: {
    FileUpload,
  ...
  methods: {
    ...
    async addItem() {
      this.loading = true;
      try {
        const res = await this.$agencyWalletSmsTransactionRepository.create({
          ...this.item,
    ...
    async editItem() {
      ...
      try {
        const res = await this.$agencyWalletSmsTransactionRepository.update({
          ...this.item,
          real_price: this.realPrice,
          user_id: this.agency.id
        });
...
export default RechargeFormDialog;
export type RechargeFormDialogRef = InstanceType<typeof RechargeFormDialog>;
</script>

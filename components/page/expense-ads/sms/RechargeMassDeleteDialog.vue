...
const RechargeMassDeleteDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "RechargeMassDeleteDialog",
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$agencyWalletSmsTransactionRepository.massDelete({
          ids: this.ids
        });
        this.loading = false;
...
export default RechargeMassDeleteDialog;
export type RechargeMassDeleteDialogRef = InstanceType<
  typeof RechargeMassDeleteDialog
>;
</script>

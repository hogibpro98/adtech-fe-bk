  ...
  >
    ...
    <template>
      ...
      <v-flex xs12 class="form-group">
        <charge-expense-sms-table
          v-if="item.type === 'charge'"
          ref="table"
          resource="/transaction-sms-vendor/daily/charge"
          :filter-custom="filterTableCharge"
        />
        <recharge-expense-sms-table
          v-if="item.type === 'recharge'"
          resource="/transaction-sms-vendor/daily/recharge"
...
import ChargeExpenseSmsTable from "~/components/page/expense-ads/sms/ChargeExpenseSmsTable.vue";
...
const ExpenseSMSVendorDetailDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "ExpenseSMSVendorDetailDialog",
  components: {
    ChargeExpenseSmsTable,
  ...
  computed: {
    ...
    filterTableCharge() {
      return {
        select_agency_id: this.item?.agency_id,
        select_agency_code: this.item?.agency_code
      };
    }
  },
  beforeDestroy() {
  ...
  methods: {
    show(item = null) {
      ...
      this.title = `Chi tiết giao dịch ${this.$filters.date(item?.date)}`;
      this.loading = false;
...
export default ExpenseSMSVendorDetailDialog;
export type ExpenseSMSDetailDialogRef = InstanceType<
  typeof ExpenseSMSVendorDetailDialog
>;
</script>

  ...
  <v-container grid-list-xl fluid class="no-padding no-margin">
    <v-layout row wrap>
      <v-flex lg12>
        ...
        <expense-s-m-s-vendor-detail-dialog ref="expenseSMSDetailDialog" />
      </v-flex>
    </v-layout>
    <export-dialog
      ref="exportDialog"
...
import ExpenseSMSVendorDetailDialog from "~/components/page/expense-ads/sms-vendor/ExpenseSMSVendorDetailDialog.vue";
...
>).extend({
  ...
  props: {
    resource: {
      type: String,
      default: TRANSACTION_SMS_VENDOR_DAILY
    },
    toolbar: {
      type: Object,
      default: () => ({
    ...
    async getSummary(params) {
      try {
        const { data } = await this.$axios.$post(
          `${TRANSACTION_SMS_VENDOR_DAILY}/summary`,
          params
        );

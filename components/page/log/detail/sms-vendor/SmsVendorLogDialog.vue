...
const SmsVendorLogDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "SmsVendorLogDialog",
  props: {},
  data() {
    return {
      ACTION_MAP,
      ...
      PROP_MAP: {
        ...
        "metadata.max_sms_per_day": {
          text: "số tin nhắn tối đa mỗi ngày",
          filter: "numberSpace"
        },
        "metadata.max_sms": {
          text: "số tin nhắn tối đa",
          filter: "numberSpace"
        },
        "metadata.schedule": {
          text: "lịch gửi"
  ...
  methods: {
    ...
    renderChanges(item) {
      if (item.action_type !== "edit") {
        ...
        return `${action} tin nhắn SMS Ngoại mạng`;
      }
      const change =
...
export default SmsVendorLogDialog;
export type SmsLogDialogRef = InstanceType<typeof SmsVendorLogDialog>;
</script>

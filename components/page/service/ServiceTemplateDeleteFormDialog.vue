...
const ServiceTemplateDeleteDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "ServiceTemplateDeleteDialog",
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$axios.post("/service/delete", {
          id: this.item.id
        });
        this.loading = false;
        if (res.status === 200) {
          ...
          this.$toast.global.delete_success("dịch vụ");
        } else {
...
export default ServiceTemplateDeleteDialog;
export type SmsTemplateDeleteDialogRef = InstanceType<
  typeof ServiceTemplateDeleteDialog
>;
</script>

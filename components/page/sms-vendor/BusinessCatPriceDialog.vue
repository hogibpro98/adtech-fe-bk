        ...
        <v-row class="mt-3 mr-3">
          ...
          <v-col class="col-1 justify-content-center d-flex">
            <v-tooltip top>
              ...
              <span>Thêm mới cấu hình giá theo nhà mạng</span>
            </v-tooltip>
          </v-col>
        </v-row>
        <v-row
          v-for="(alias, index) in item.business_cat_price"
          :key="index"
        ...
        >
          <v-col class="w-45-percent" xs6 md6>
            <telco-select
              ...
              :loading="loading"
              :name="`business_cat_price.${index}.telco_code`"
              :return-object="false"
              label="Nhà mạng"
              item-value="code"
              required
          ...
          <v-col class="w-45-percent" xs6 md6>
            <v-number-field-with-validation
              v-model="alias.price"
              :loading="loading"
              name="`business_cat_price.${index}.price`"
              type="text"
              label="Giá tiền"
              suffix="₫"
              required
...
const BusinessCatPriceDialog = Vue.extend({
  name: "BusinessCatPriceDialog",
  components: {
    FormDialog,
  ...
  computed: {
    title() {
      return "Cập nhật cấu hình giá theo nhà mạng";
    },
    labelButtonConfirm() {
      return "Xác nhận";
    }
  ...
  methods: {
    ...
    async editItem() {
      ...
      const data = {
        id: this.item.id,
        business_cat_price: this.item.business_cat_price
      };
      try {
        const result = await this.$axios.$post(
          "business-cat/update-business-cat-prices",
          data
        );
        if (result.code === CODE_SUCCESS) {
          this.$emit("action-success");
          this.$toast.global.action_success(
            "cập nhật cấu hình giá theo nhà mạng"
          );
          this.hide();
        } else {
    ...
    handleDelete(index) {
      this.item.business_cat_price.splice(index, 1);
    },
    show(item) {
      if (item) {
        this.item = cloneDeep(item);
        if (!item.business_cat_price) {
          this.$set(this.item, "business_cat_price", [
            {
              telco_code: "",
              price: ""
            }
    ...
    handleAddConfig() {
      this.item.business_cat_price.push({
        telco_code: "",
        price: ""
      });
    }
...
export default BusinessCatPriceDialog;
export type BusinessCatPriceDialogRef = InstanceType<
  typeof BusinessCatPriceDialog
>;
</script>

...
const MmsTestDialog = (Vue as VueConstructor<
...
>).extend({
  ...
  methods: {
    ...
    async confirmItem() {
      this.loading = true;
      try {
        const res = await this.$adsMMSRepository.post("test-message", {
          ads_id: this.item.id,
          phone: this.phone,
          params: this.params
        });
...
export default MmsTestDialog;
export type MmsTestDialogRef = InstanceType<typeof MmsTestDialog>;
</script>

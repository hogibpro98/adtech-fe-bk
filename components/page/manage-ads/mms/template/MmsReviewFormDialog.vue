<template>
  <v-dialog
    ...
    no-click-animation
    persistent
    content-class="flex xs12 md8 xl5"
    v-on="$listeners"
  >
    <v-card>
      <v-card-title>
        <span class="headline">Phê duyệt template MMS</span>
      </v-card-title>
      <v-card-text>
        <v-container>
          <v-row
          ...
          >
            <v-flex sm7 class="form-group pr-3 mt-4">
              <v-list-item dense>
                Người sở hữu: &nbsp;
              ...
              <v-list-item dense>
                Template ID: &nbsp;<strong> {{ item.tempid }}</strong>
              </v-list-item>
              <v-list-item dense>
                Tên brandname: &nbsp;<strong>
                  {{ item.brandname && item.brandname.name }}</strong
                >
              </v-list-item>
              <v-list-item dense>
              ...
              <v-list-item dense>
                Đơn giá: &nbsp;<strong> {{ item.price | currency }}</strong>
              </v-list-item>
            </v-flex>
            <v-flex sm5 class="form-group ">
          ...
          <v-layout justify-end class="mt-0  z-10 background px-8 py-2">
            ...
            <v-btn
              v-if="canReject"
              :loading="isLoadingRejectBtn"
              :disabled="isLoadingConfirmBtn"
            ...
            <v-btn
              v-if="canApprove"
              :loading="isLoadingConfirmBtn"
              :disabled="isLoadingRejectBtn"
...
const MmsTemplateReviewDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "MmsTemplateReviewDialog",
  components: {
  ...
  methods: {
    ...
    async approve() {
      ...
      try {
        const res = await this.$templateMmsRepository.post("approve", {
          id: this.item.id
        });
        this.isLoadingConfirmBtn = false;
        if (res.code === CODE_SUCCESS) {
          this.showing = false;
          this.$emit("success");
          this.$toast.global.action_success("phê duyệt MMS Template");
        } else {
    ...
    async reject() {
      this.isLoadingRejectBtn = true;
      try {
        const res = await this.$templateMmsRepository.post("reject", {
          id: this.item.id
        });
        this.isLoadingRejectBtn = false;
        if (res.code === CODE_SUCCESS) {
          this.showing = false;
          this.$emit("success");
          this.$toast.global.action_success("từ chối phê duyệt MMS Template");
        } else {
          this.$toast.global.action_fail();
        }
      } catch (e) {
...
export default MmsTemplateReviewDialog;
export type MmsTemplateReviewDialogRef = InstanceType<
  typeof MmsTemplateReviewDialog
>;
</script>

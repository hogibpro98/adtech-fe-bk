<template>
  <confirm-dialog
    ...
    :loading="loading"
    title="Bạn muốn phê duyệt template MMS?"
...
const MmsTemplateMassApproveDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "MmsTemplateMassApproveDialog",
  ...
  methods: {
    ...
    async approve() {
      ...
      try {
        const res = await this.$templateMmsRepository.massApprove({
          ids: this.ids
        });
        this.loading = false;
        if (res.code === CODE_SUCCESS) {
          ...
          this.$toast.global.action_success("duyệt template MMS");
        } else {
...
export default MmsTemplateMassApproveDialog;
export type CampaignDeleteDialogRef = InstanceType<
  typeof MmsTemplateMassApproveDialog
>;
</script>

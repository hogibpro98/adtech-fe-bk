...
const MmsTemplateDeleteDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "MmsTemplateDeleteDialog",
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$templateMmsRepository.delete({
          id: this.item.id
        });
        this.loading = false;
...
export default MmsTemplateDeleteDialog;
export type MmsTemplateDeleteDialogRef = InstanceType<
  typeof MmsTemplateDeleteDialog
>;
</script>

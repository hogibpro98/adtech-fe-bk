...
const SaveTemplateDialog = (Vue as VueConstructor<
...
>).extend({
  ...
  methods: {
    ...
    async confirmItem() {
      this.loading = true;
      try {
        const res = await this.$templateMmsRepository.post("create-from-ads", {
          id: this.item.id
        });
        this.loading = false;
...
export type SaveTemplateDialogRef = InstanceType<typeof SaveTemplateDialog>;
</script>

<template>
  <confirm-dialog
    ...
    :loading="loading"
    title="Bạn muốn phê duyệt template SMS ngoại mạng?"
...
const SmsVendorTemplateMassApproveDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "SmsVendorTemplateMassApproveDialog",
    ...
    async approve() {
      ...
      try {
        const { data } = await this.$axios.post(
          "template-sms-vendor/mass-approve",
          {
            ids: this.ids
          }
        );
        ...
        if (data.code === CODE_SUCCESS) {
          ...
          this.$toast.global.action_success(
            "duyệt template tin nhắn ngoại mạng"
          );
        } else {
...
export default SmsVendorTemplateMassApproveDialog;
export type CampaignDeleteDialogRef = InstanceType<
  typeof SmsVendorTemplateMassApproveDialog
>;
</script>

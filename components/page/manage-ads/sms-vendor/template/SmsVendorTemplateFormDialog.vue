<template>
  ...
  >
    <v-card>
      <v-card-title>
        <span class="headline"
          >{{ isEdit ? "Cập nhật" : "Thêm" }} template SMS ngoại mạng</span
        >
      </v-card-title>
      <v-card-text>
        <v-container>
...
const SmsVendorTemplateFormDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "SmsVedorTemplateFormDialog",
  components: {
    BusinessCatSelect,
    UserSelect,
  ...
  methods: {
    async show(item = null) {
      if (item) {
        this.isEdit = true;
        const brandname = cloneDeep(item.brandname);
        ...
        await this.$nextTick();
        await this.$nextTick();
        this.item.brandname = brandname;
      }

      this.showing = true;
    },

    async clone(item = null) {
      if (item) {
        const brandname = cloneDeep(item.brandname);
    ...
    async addItem() {
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          this.$toast.global.add_success("template SMS ngoại mạng");
        } else {
    ...
    async editItem() {
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          this.$toast.global.edit_success("template tin nhắn ngoại mạng");
        } else {
          this.$toast.global.action_fail();
        }
      } catch (e) {
    ...
    resetData() {
      this.timeout = setTimeout(() => {
        this.isEdit = false;
        this.item = cloneDeep(DEFAULT_ITEM);console.log(this.item);
...
export default SmsVendorTemplateFormDialog;
export type SmsVendorTemplateFormDialogRef = InstanceType<
  typeof SmsVendorTemplateFormDialog
>;
</script>

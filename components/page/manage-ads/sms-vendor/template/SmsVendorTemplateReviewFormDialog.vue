<template>
  <v-dialog
    ...
    no-click-animation
    persistent
    content-class="flex xs12 md7 xl6"
    v-on="$listeners"
  >
    <v-card>
      <v-card-title>
        <span class="headline">Phê duyệt template SMS ngoại mạng</span>
      </v-card-title>
      <v-card-text>
        <v-container>
          <v-row
...
const SmsTemplateReviewDialog = (Vue as VueConstructor<Vue & ComponentData & ComponentMethods>).extend({
  name: "SmsTemplateReviewDialog",
  components: {
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          this.$toast.global.action_success(
            "phê duyệt template SMS ngoại mạng"
          );
        } else {
    ...
    async reject() {
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          this.$emit("success");
          this.$toast.global.action_success(
            "từ chối phê duyệt template SMS ngoại mạng"
          );
        } else {
          this.$toast.global.action_fail();
        }
...
export default SmsTemplateReviewDialog;
export type SmsTemplateReviewDialogRef = InstanceType<typeof SmsTemplateReviewDialog>;
</script>

<template>
  <confirm-dialog
    ref="dialog"
    :loading="loading"
    title="Hướng dẫn API Template SMS Vendor"
...
export default (Vue as VueConstructor<Vue & ComponentData>).extend({
  name: "ApiInstructionTemplateSmsVendorDialog",
  ...
  computed: {
    request() {
      return `POST ${process.env.API_ENDPOINT}vendor/sms-temp/send`;
    },
    params() {
      const params = {};

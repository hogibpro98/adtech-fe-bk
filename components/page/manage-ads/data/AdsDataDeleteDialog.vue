<template>
  ...
  >
    <template slot="content">
      Bạn có chắc chắn muốn xóa tài trợ data
...
const AdsDataDeleteDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "AdsDataDeleteDialog",
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$adsRepository.delete({
          id: this.item.id
        });
        this.loading = false;
...
export default AdsDataDeleteDialog;
export type AdsDataDeleteDialogRef = InstanceType<typeof AdsDataDeleteDialog>;
</script>

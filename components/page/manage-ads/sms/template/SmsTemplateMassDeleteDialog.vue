<template>
  <confirm-dialog
    ...
    :loading="loading"
    title="Bạn muốn xóa template SMS?"
  ...
  >
    <template slot="content">
      ...
      <div>Danh sách template SMS sẽ bị xóa:</div>
      <ul>
        <li
...
const SmsTemplateMassDeleteDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "SmsTemplateMassDeleteDialog",
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$templateSmsRepository.massDelete({
          ids: this.ids
        });
        this.loading = false;
        if (res.code === CODE_SUCCESS) {
          ...
          this.$toast.global.delete_success("template SMS");
        } else {
...
export default SmsTemplateMassDeleteDialog;
export type SmsTemplateMassDeleteDialogRef = InstanceType<
  typeof SmsTemplateMassDeleteDialog
>;
</script>

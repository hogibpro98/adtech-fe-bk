      ...
      >
        <template slot="item.name" slot-scope="{ item }">
          <v-layout class="my-0">
            ...
            <nuxt-link :to="`/manage-ads/sms/${item.id}`">
              {{ item.name }}
            </nuxt-link>
          </v-layout>
        </template>
        ...
        <template slot="item.brand_name" slot-scope="{ item }">
          <nuxt-link :to="`/manager-brandname/${item.brandname.id}`">
            {{ item.brandname.name }}
          </nuxt-link>
        </template>
...
>).extend({
  ...
  methods: {
    ...
    getRunDuration(item) {
      const endTime =
        moment(item.end_time, SQL_TIMESTAMP).unix() < this.currentTime
          ? moment(item.end_time, SQL_TIMESTAMP).unix()
          : this.currentTime;
      return endTime - moment(item.start_time).unix() - item.total_time_pause;
    },
    checkHasApi(item) {

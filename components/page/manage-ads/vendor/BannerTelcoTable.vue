<template>
  <v-flex lg12 class="no-padding">
      ...
      >
        <template slot="item.name" slot-scope="{ item }">
          <v-layout class="my-0">
            ...
            <nuxt-link :to="`/manage-ads/list-ads-sms-vendor/${item.id}`"
              >{{ item.name }}
            </nuxt-link>
          </v-layout>
        </template>
        ...
        <template slot="item.total_time_running" slot-scope="{ item }">
          <div class="text-right">{{ getRunDuration(item) | humanize }}</div>
        </template>
        <template
    ...
    <banner-telco-form-dialog
      v-if="isCreateBanner || isEditBanner"
      ref="bannerTelcoDialog"
      v-on="$listeners"
    ...
    <banner-telco-form-review-dialog
      ref="bannerTelcoReviewDialog"
      @action-success="refreshTable"
    >
    </banner-telco-form-review-dialog>
    <banner-telco-form-review-dialog
    ...
    <failed-telco-messages-dialog ref="failedDialog" />
    <mms-banner-mass-delete-dialog
      ref="massDeleteDialog"
      @confirm="deleteSelected"
      @action-success="refreshTable(false, false)"
    />
    <banner-sms-vendor-approve-multi-dialog
      ref="approveMultiDialog"
      @action-success="refreshTable"
    ></banner-sms-vendor-approve-multi-dialog>
    <reject-multi-dialog
      ref="rejectMultiDialog"
      object-name="tin nhắn ngoại mạng"
      resource="/ads"
      @action-success="refreshTable"
    />
    <telco-test-dialog ref="testDialog" @action-success="refreshTable" />
    <api-instruction-dialog ref="apiDialog" service="vendor/sms" />
...
import BannerMMSFormReviewDialog from "@/components/page/manage-campaign/banner/BannerMMSFormReviewDialog.vue";
...
import BannerMMSFormDialog from "@/components/page/manage-campaign/banner/BannerMMSFormDialog.vue";
...
import {
  ...
  STATUS_BANNER_SMS_VENDOR_FILTER,
  SELECT_MMS_TYPE
} from "@/constants/dataSelect";
import { DATE, NUMBER, RESOURCE, SELECT } from "@/constants/filterType";
import { ADS_MMS, ADS_SMS_VENDOR, ADVERTISER_MMS } from "@/constants/resource";
...
import TelcoTestDialog from "~/components/page/manage-ads/vendor/TelcoTestDialog.vue";
...
import BannerTelcoFormDialog from "~/components/page/manage-ads/vendor/index/BannerTelcoFormDialog.vue";
import BannerTelcoFormReviewDialog from "~/components/page/manage-ads/vendor/BannerTelcoFormReviewDialog.vue";
import BannerSmsVendorApproveMultiDialog from "~/components/page/manage-campaign/banner/BannerSmsVendorApproveMultiDialog.vue";
import CampaignSelect from "~/components/select/CampaignSelect.vue";
import FailedTelcoMessagesDialog from "~/components/page/manage-ads/vendor/FailedTelcoMessagesDialog.vue";
...
interface ComponentData extends GlobalDataInterface {
  ...
  filterAll: object;
  $refs: {
    bannerTelcoDialog: BannerMMSFormDialogRef;
    deleteDialog: BannerDeleteDialogRef;
    bannerTelcoReviewDialog: BannerMMSFormReviewDialogRef;
  ...
  $adsSmsVendorRepository: RepositoryInterface;
}
...
interface ComponentMethod {
  ...
  renderSMSType(item: BannerMMSInterface): string;
...
>).extend({
  name: "BannerTelcoTable",
  components: {
    BannerSmsVendorApproveMultiDialog,
    TelcoTestDialog,
    BannerTelcoFormReviewDialog,
    BannerTelcoFormDialog,
    SaveTemplateDialog,
    ServiceTargetingConfigDialog,
    ApiInstructionDialog,
    MmsTestDialog,
    FailedMmsMessageDialog,
    RejectMultiDialog,
    MmsBannerMassDeleteDialog,
    BannerMMSApproveMultiDialog,
    BannerMMSTemplateFormReviewDialog,
    BannerMMSFormReviewDialog,
    BannerApproveDialog,
    BannerPauseDialog,
    BannerStartDialog,
    BannerMMSFormDialog,
  ...
  props: {
    ...
    pageName: {
      type: String,
      default: "manage-ads-list-ads-sms-vendor"
    }
  },
  data() {
    return {
      ...
      filters: [
        ...
        },
        {
          ...
          name: "status",
          label: "Trạng thái",
          items: STATUS_BANNER_SMS_VENDOR_FILTER
        },
        {
          type: DATE,
        ...
        {
          type: RESOURCE,
          name: "sms_vendor_user_template_id",
          label: "Template tin nhắn ngoại mạng",
          itemValue: "id",
          itemText: "name",
          component: SmsVendorTemplateSelect,
          visible: false
        },
        // {
        //   type: RESOURCE,
  ...
  methods: {
    ...
    async getSummary(params) {
      ...
      try {
        const { data } = await this.$adsSmsVendorRepository.post(
          "summary",
          params
        );
        this.summary = data;
    ...
    checkCanSaveTemplate(item) {
      return (
        ...
        item?.tempid &&
        this.$can("manage-ads-mms-template.add") &&
    ...
    addItem() {
      if (this.formSelectedCampaign) {
        this.$refs.bannerTelcoDialog.show({
    ...
    async editItem(item) {
      try {
        this.loading = true;
        const res = await this.$adsSmsVendorRepository.getDetail(item.id);
        this.loading = false;
        if (res.code === CODE_SUCCESS && res.data) {
          this.$refs.bannerTelcoDialog.show(res.data);
        } else {
    ...
    async copyBanner(item) {
      try {
        ...
        if (res.code === CODE_SUCCESS && res.data) {
          ...
          this.$refs.bannerTelcoDialog.show(data);
        } else {
          this.$toast.global.action_fail();
        }
      } catch (e) {
    ...
    async reviewItem(item) {
      try {
        ...
        this.loading = false;
        if (res.code === CODE_SUCCESS && res.data) {
          this.$refs.bannerTelcoReviewDialog.show(res.data);
        } else {
          this.$toast.global.action_fail();
        }
      } catch (e) {
    ...
    openConfig() {
      this.$refs.configDialog.show("sms-vendor", "tin nhắn ngoại mạng");
    }
  }
});
</script>

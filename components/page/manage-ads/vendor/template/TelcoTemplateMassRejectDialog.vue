<template>
  <confirm-dialog
    ...
    :loading="loading"
    title="Bạn muốn từ chối template SMS?"
...
const TelcoTemplateMassRejectDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "TelcoTemplateMassRejectDialog",
  ...
  methods: {
    ...
    async approve() {
      ...
      try {
        const res = await this.$templateSmsRepository.massReject({
          ids: this.ids
        });
        this.loading = false;
        if (res.code === CODE_SUCCESS) {
          ...
          this.$toast.global.action_success("từ chối template SMS");
        } else {
...
export default TelcoTemplateMassRejectDialog;
export type CampaignDeleteDialogRef = InstanceType<
  typeof TelcoTemplateMassRejectDialog
>;
</script>

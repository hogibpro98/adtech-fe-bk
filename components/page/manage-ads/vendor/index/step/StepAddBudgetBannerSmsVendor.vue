            ...
            <v-row class="no-margin ml-34">
              <v-flex sm4>
                <v-number-field-with-validation
                  ...
                  v-model="innerValue.max_sms_per_day"
                  :loading="loading"
                  :decimal="false"
...
const StepAddBudgetBannerSmsVendor = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "StepAddBudgetBannerSmsVendor",
  ...
  computed: {
    ...
    numMMSOfDay() {
      ...
      if (this.typeBudget === "unlimit") {
        numMMSOfDay = Math.floor(this.innerValue.max_sms / this.numDayBanner);
      } else {
        numMMSOfDay = this.innerValue.max_sms_per_day;
      }
    ...
    numMMSTotal() {
      let numMMSTotal = 0;
      if (this.typeBudget === "unlimit") {
        numMMSTotal = this.innerValue.max_sms;
      } else {
        const numTotalPerDay = this.innerValue.max_sms_per_day;
...
export default StepAddBudgetBannerSmsVendor;
export type StepAddBudgetBannerSmsVendorRef = InstanceType<
  typeof StepAddBudgetBannerSmsVendor
>;
</script>

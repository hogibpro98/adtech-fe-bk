<template>
  ...
  >
    ...
    <template slot="content-2">
      <step-information-banner-sms-vendor
        v-model="item"
        :config="config"
        :loading="loading"
      />
    ...
    <template slot="content-7">
      <step-add-budget-banner-sms-vendor v-model="item" :loading="loading" />
    </template>
...
import FormDialogStep, {
  FormDialogStepRef
} from "~/components/common/FormDialogStep/index.vue";
...
import { STEPS_BANNER_SMS_VENDOR } from "~/constants/dataSelect";
...
import StepInformationBannerSmsVendor from "~/components/page/manage-ads/vendor/index/step/StepInformationBannerSmsVendor.vue";
import StepAddBudgetBannerSmsVendor from "~/components/page/manage-ads/vendor/index/step/StepAddBudgetBannerSmsVendor.vue";
import StepTargetingBannerVendor from "~/components/page/manage-ads/vendor/index/step/StepTargetingBannerVendor.vue";
...
const BannerTelcoFormDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "BannerTelcoFormDialog",
  components: {
    StepTargetingBannerVendor,
    StepAddBudgetBannerSmsVendor,
    StepInformationBannerSmsVendor,
  ...
  computed: {
    ...
    steps() {
      console.log(this.isDynamic, this.hasParamsFile);
      if (this.isDynamic) {
        return STEPS_BANNER_SMS_VENDOR.map((step, index) => {
          let disabled = false;
      ...
      if (this.targetingType === "import_file") {
        ...
        return STEPS_BANNER_SMS_VENDOR;
      }
    }
  },
  watch: {
  ...
  methods: {
    ...
    async show(item = null) {
      ...
      if (item && !item.autoFillCampaign) {
        ...
        if (!item.id) {
          ...
          this.$refs.refFormDialogStep.show(STEPS_BANNER_SMS_VENDOR.length);
          this.$refs.refFormDialogStep.setStepComplete(
            STEPS_BANNER_SMS_VENDOR[STEPS_BANNER_SMS_VENDOR.length - 1].index
          );
          return;
        }
      } else {
      ...
      this.title = `${this.isEdit ? "Cập nhật" : "Thêm"} tin nhắn ngoại mạng`;
      this.type = "general";
    ...
    onSubmit(step) {
      ...
      } else if (step === STEPS_BANNER_SMS_VENDOR.length) {
        if (this.isEdit) {
          this.editItem();
        } else {
          this.addItem();
    ...
    async addItem() {
      this.loading = true;
      try {
        const data = convertDataToUpdateAdsSms(this.item);
        data.sms_vendor_user_template_id = data.sms_user_template_id;
        await this.$nextTick();
        const res = await this.$adsSmsVendorRepository.create(data);
        this.loading = false;
        if (res.code === CODE_SUCCESS) {
          this.hidden();
    ...
    async editItem() {
      ...
      try {
        ...
        data.sms_vendor_user_template_id = data.sms_user_template_id;
        await this.$nextTick();
        const res = await this.$adsSmsVendorRepository.update(data);
        this.loading = false;
        if (res.code === CODE_SUCCESS) {
          this.hidden();
...
export default BannerTelcoFormDialog;
export type BannerSmsFormDialogRef = InstanceType<typeof BannerTelcoFormDialog>;
</script>

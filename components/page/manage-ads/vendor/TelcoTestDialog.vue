<template>
  <form-dialog
    ...
    :loading="loading"
    title="Gửi thử tin nhắn ngoại mạng"
...
const TelcoTestDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "TelcoTestDialog",
  components: {
  ...
  methods: {
    ...
    async confirmItem() {
      this.loading = true;
      try {
        const res = await this.$axios.post("ads-sms-vendor/test-message", {
          ads_id: this.item.id,
          phone: this.phone,
          params: this.params
        });
...
export default TelcoTestDialog;
export type TelcoTestDialogRef = InstanceType<typeof TelcoTestDialog>;
</script>

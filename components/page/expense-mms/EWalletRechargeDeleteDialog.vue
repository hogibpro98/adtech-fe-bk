...
const EWalletRechargeDeleteDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "EWalletRechargeDeleteDialog",
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$agencyWalletTransactionRepository.delete({
          id: this.item.id
        });
        this.loading = false;
...
export default EWalletRechargeDeleteDialog;
export type EWalletRechargeDeleteDialogRef = InstanceType<
  typeof EWalletRechargeDeleteDialog
>;
</script>

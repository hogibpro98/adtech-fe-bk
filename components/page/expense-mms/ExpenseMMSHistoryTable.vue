  ...
  <v-container grid-list-xl fluid class="no-padding no-margin">
    <v-layout row wrap>
      <v-flex lg12>
        ...
        <expense-m-m-s-detail-dialog ref="expenseMMSDetailDialog" />
      </v-flex>
    </v-layout>
    <export-dialog
      ref="exportDialog"
...
import ExpenseMMSDetailDialog from "@/components/page/expense-mms/ExpenseMMSDetailDialog.vue";
...
import { TRANSACTION_MMS_DAILY } from "@/constants/resource";
...
import { ExpenseMMSDetailDialogRef } from "~/components/page/expense-mms/ExpenseMMSDetailDialog.vue";
...
interface ComponentData extends GlobalDataInterface {
  ...
  $refs: {
    ...
    expenseMMSDetailDialog: ExpenseMMSDetailDialogRef;
  };
}
...
>).extend({
  name: "ExpenseMMSHistoryTable",
  components: {
    ExportDialog,
    OverviewPanel,
    ExpenseMMSDetailDialog,
  ...
  methods: {
    async getSummary(params) {
      try {
        const { data } = await this.$transactionMMSDailyRepository.post(
          "summary",
          params
        );
    ...
    showDialogDetail(item) {
      this.$refs.expenseMMSDetailDialog.show(item);
    },

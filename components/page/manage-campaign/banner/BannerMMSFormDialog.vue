<template>
  ...
  >
    ...
    <template slot="content-7">
      <step-add-budget-banner-m-m-s v-model="item" :loading="loading">
      </step-add-budget-banner-m-m-s>
    </template>
...
const BannerMMSFormDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "BannerMMSFormDialog",
  components: {
    CodeBlock,
  ...
  methods: {
    ...
    async show(item = null) {
      ...
      if (item && !item.autoFillCampaign) {
        ...
        this.$store.commit("shorten/ADD_CONTENT_URL", item.content.data);
      ...
      this.title = `${this.isEdit ? "Cập nhật" : "Thêm"} tin nhắn MMS`;
      this.type = "general";
    ...
    async addItem() {
      this.loading = true;
      try {
        const res = await this.$adsMMSRepository.create(
          convertDataToUpdateAdsMMS(this.item)
        );
        this.loading = false;
        if (res.code === CODE_SUCCESS) {
          this.hidden();
    ...
    async editItem() {
      ...
      try {
        const res = await this.$adsMMSRepository.update(
          convertDataToUpdateAdsMMS(this.item)
        );
        this.loading = false;
        if (res.code === CODE_SUCCESS) {
    ...
    async reloadDataBanner() {
      ...
      if (bannerId) {
        const res = await this.$adsMMSRepository.getDetail(bannerId);
...
export default BannerMMSFormDialog;
export type BannerMMSFormDialogRef = InstanceType<typeof BannerMMSFormDialog>;
</script>

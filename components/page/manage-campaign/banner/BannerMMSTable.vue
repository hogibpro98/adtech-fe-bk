<template>
  <v-flex lg12 class="no-padding">
    ...
    ></banner-delete-dialog>
    <banner-m-m-s-form-review-dialog
      ref="bannerMMSReviewDialog"
      @action-success="refreshTable"
    ></banner-m-m-s-form-review-dialog>
    <banner-m-m-s-template-form-review-dialog
    ...
    <failed-mms-message-dialog ref="failedDialog" />
    <mms-banner-mass-delete-dialog
      ref="massDeleteDialog"
      @confirm="deleteSelected"
      @action-success="refreshTable(false, false)"
    />
    <banner-m-m-s-approve-multi-dialog
      ref="approveMultiDialog"
      @action-success="refreshTable"
    ></banner-m-m-s-approve-multi-dialog>
    <reject-multi-dialog
      ref="rejectMultiDialog"
      object-name="quảng cáo tin nhắn MMS"
      resource="/ads"
      @action-success="refreshTable"
    />
    <mms-test-dialog ref="testDialog" @action-success="refreshTable" />
    <api-instruction-dialog ref="apiDialog" />
...
interface ComponentData extends GlobalDataInterface {
  ...
  $refs: {
    ...
    bannerMMSReviewDialog: BannerMMSFormReviewDialogRef;
  ...
  methods: {
    ...
    async editItem(item) {
      try {
        this.restShortUrlStore();
        this.loading = true;
        const res = await this.$adsMMSRepository.getDetail(item.id);
        this.loading = false;
    ...
    async reviewItem(item) {
      try {
        ...
        this.loading = false;
        if (res.code === CODE_SUCCESS && res.data) {
          this.$refs.bannerMMSReviewDialog.show(res.data);
        } else {
          this.$toast.global.action_fail();
        }
      } catch (e) {

        ...
        <v-card outlined class="preview-banner-content no-border">
          ...
          <div class="preview-sms-wrap">
            ...
            <template v-for="({ type, data }, index) in mediaData">
              ...
              <div
                v-if="type === 'video' && data"
                :key="`media-mms-${type}-${index}-${data}`"
                class="preview-sms-item"
              >
                <video
                  width="100%"
                  height="100%"
                  controls
                  class="preview-sms-item-video media"
                >
                  <source :src="data" />
...
const PreviewBannerMms = (Vue as VueConstructor).extend({
  name: "PreviewBannerMms",
  props: {
    displaySize: {
      type: Object,
      default: () => ({
  ...
  methods: {
    ...
    generateTitleForLink(string) {
      ...
      return resultData;
      // } else {
      //   resultData = this.getLinkAndText(string);
      //   return resultData;
      // }
    }
  }
...
export default PreviewBannerMms;
export type PreviewBannerMmsRef = InstanceType<typeof PreviewBannerMms>;
</script>

<template>
  <v-row class="step-preview-banner no-margin">
        ...
        <v-row v-if="showTemplateId" class="no-margin">
          <v-list-item-content>
            ...
            <v-list-item-subtitle class="v-content-review">
              {{ innerValue.tempid || "Chưa có template id" }}
      ...
      <v-flex xs12>
        <v-row class="no-margin">
          <v-list-item-content>
            ...
            <v-list-item-subtitle
              v-if="businessActivity"
              class="v-content-review"
            >
              Lĩnh vực hoạt động: {{ innerValue.cat_id }}
            </v-list-item-subtitle>
            <v-list-item-subtitle class="v-content-review">
              Thời gian diễn ra: {{ dateRunAdsMMS }}
            </v-list-item-subtitle>
            <v-list-item-subtitle class="v-content-review">
        ...
        <v-row class="no-margin">
          <v-list-item-content>
            ...
            <v-list-item-subtitle class="v-content-review">
              Nhóm thuê bao:
              <a
                v-if="innerValue.phone_group"
                href="javascript:;"
                @click="showPhoneList"
              >
                {{ innerValue.phone_group.name }}
              </a>
            </v-list-item-subtitle>
          </v-list-item-content>
          <v-icon
    ...
    <banner-m-m-s-edit-information-dialog
      ...
      @submit="onSaveData"
    />
    <banner-m-m-s-edit-budget-dialog
    ...
    <banner-edit-targeting-optional-dialog
      ...
      @submit="onSaveData"
    />
    <banner-m-m-s-edit-type-targeting-dialog
      ref="refBannerEditTypeTargetingDialog"
      @submit="onSaveData"
    />
    <confirm-dialog
      v-if="phone_group && innerValue.phone_group.numbers"
...
<script lang="ts">
import PreviewContentBannerMMS from "@/components/page/manage-campaign/banner/common/PreviewContentBannerMMS.vue";
...
import BannerMMSEditBudgetDialog, {
  BannerMMSEditBudgetDialogRef
} from "@/components/page/manage-campaign/banner/dialog-edit-step/BannerMMSEditBudgetDialog.vue";
import BannerMMSEditInformationDialog, {
  BannerMMSEditInformationDialogRef
} from "@/components/page/manage-campaign/banner/dialog-edit-step/BannerMMSEditInformationDialog.vue";
import BannerMMSEditTypeTargetingDialog, {
  BannerMMSEditTypeTargetingDialogRef
} from "@/components/page/manage-campaign/banner/dialog-edit-step/BannerMMSEditTypeTargetingDialog.vue";
...
const StepReviewBannerMMS = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethods
>).extend({
  name: "StepReviewBannerMMS",
  components: {
    ConfirmDialog,
    BannerMMSEditInformationDialog,
    BannerMMSEditTypeTargetingDialog,
    BannerMMSEditBudgetDialog,
    PreviewContentBannerMMS,
  ...
  computed: {
    ...
    timeSendMMS() {
      let msg = "";
      if (Array.isArray(this.innerValue.schedule)) {
        this.innerValue.schedule.forEach(e => {
          const days = e.days
            .map(day => {
    ...
    isPermissionEdit() {
      ...
      return this.$can(`manage-ads-list-ads-mms.${ACTIONS.EDIT}`);
    },
    isPermissionShowPrice() {
      return this.$can(`manage-ads-list-ads-mms.${ACTIONS.SHOW_PRICE}`);
    },
    readonly() {
      if (this.value?.id) {
    ...
    numMMSTotal() {
      ...
      if (this.innerValue?.budget_mode === "unlimit") {
        numMMSTotal = this.innerValue.max_mms || 0;
      } else {
        const numTotalPerDay = this.innerValue.max_mms_per_day || 0;
    ...
    numMMSOfDay() {
      ...
      if (this.innerValue?.budget_mode === "unlimit") {
        ...
        numMMSOfDay = this.innerValue.max_mms_per_day || 0;
      }
    ...
    priceMMSName() {
      return SELECT_MMS_TYPE.find(
        type => parseInt(this.innerValue.mms_type, 10) === type.value
      )?.text;
    },
    pricingModeValue() {
      return `${this.$filters.currency(this.priceMMS)}/tin nhắn`;
    },
    displaySize() {
...
export default StepReviewBannerMMS;
export type StepReviewBannerMMSRef = InstanceType<typeof StepReviewBannerMMS>;
</script>

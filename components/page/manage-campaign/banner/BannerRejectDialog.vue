<template>
  <confirm-dialog
    ...
    :loading="loading"
    title="Bạn muốn từ chối quảng cáo?"
    label-button-confirm="Xác nhận"
    @confirm="confirmItem"
  >
    <template slot="content">
      <v-flex xs12>
        Bạn có chắc chắn muốn từ chối quảng cáo
...
const BannerRejectDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "BannerRejectDialog",
    ...
    async confirmItem() {
      ...
      try {
        ...
        if (res.code === CODE_SUCCESS) {
          ...
          this.$toast.global.action_success("từ chối quảng cáo");
        } else {
...
export default BannerRejectDialog;
export type BannerRejectDialogRef = InstanceType<typeof BannerRejectDialog>;
</script>

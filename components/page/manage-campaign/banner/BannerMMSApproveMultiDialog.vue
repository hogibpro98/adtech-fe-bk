\<template>
  <confirm-dialog
    ...
    :loading="loading"
    title="Duyệt nhiều quảng cáo tin nhắn MMS"
    label-button-confirm="Xác nhận"
    @confirm="approve"
  >
    <template slot="content">
      Bạn có chắc chắn muốn duyệt
      <span class="red--text"> {{ items.length }}</span> quảng cáo tin nhắn không?
      <ul>
        <li
...
const BannerMMSApproveMultiDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "BannerMMSApproveMultiDialog",
...
export default BannerMMSApproveMultiDialog;
export type CampaignDeleteDialogRef = InstanceType<
  typeof BannerMMSApproveMultiDialog
>;
</script>

<template>
  ...
  >
    <template slot="content">
      ...
      <span class="red--text">{{ items.length }}</span> chiến dịch không?
      <div>Danh sách chiến dịch sẽ bị xóa:</div>
      <ul>
        <li
...
const CampaignMassDeleteDialog = (Vue as VueConstructor<
  Vue & ComponentData & ComponentMethod
>).extend({
  name: "CampaignMassDeleteDialog",
    ...
    displayWarning() {
      if (this.items.length > 0) {
        ...
        if (this.items.length > 1) {
          return `${firstName} và ${this.items.length - 1} chiến dịch khác`;
        }
        return firstName;
      }
      return "";
  ...
  methods: {
    ...
    async deleteItem() {
      this.loading = true;
      try {
        const res = await this.$campaignRepository.massDelete({
          ids: this.ids
        });
        this.loading = false;
...
export default CampaignMassDeleteDialog;
export type CampaignMassDeleteDialogRef = InstanceType<
  typeof CampaignMassDeleteDialog
>;
</script>

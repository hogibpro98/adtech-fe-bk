<template>
  <validation-provider
    ...
    immediate
  >
    <v-text-field v-show="false" v-model="innerValue" />
    <v-text-field
...
import { ALL_WHITE_SPACE, DECIMAL_TYPING } from "@/constants/regex";
...
>).extend({
  name: "VNumberFieldWithSpaceValidation",
  props: {
    rules: {
      type: [Object, String],
      default: ""
  ...
  watch: {
    ...
    async innerValue(val) {
      if (val || val === 0) {
        this.formattedValue = val;
        await this.$nextTick();
        if (this.decimal) {
          this.pushDecimalToProps(val);
        } else {
  ...
  methods: {
    ...
    pushDecimalToProps(val) {
      if (!DECIMAL_TYPING.test(val)) {
        if (
          typeof val === "string" &&
          !val.endsWith(".") &&
          !ALL_WHITE_SPACE.test(val)
        ) {
          this.formattedValue = numeral(val).format(DECIMAL);
          this.$emit("input", numeral._.stringToNumber(this.formattedValue));
